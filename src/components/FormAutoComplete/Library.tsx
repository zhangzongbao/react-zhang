...
import { GetLibraryQuery } from './api';
...
const FormAutoCompleteLibrary = (props: Props) => {
  const { value, onChange, readonly, defaultValue, showTip } = props;
  const [needShowTip, setNeedShowTip] = useState(false);

  const handleSearch = async (v: string | boolean | undefined): Promise<Array<LibraryItem>> => {
    if (showTip) setNeedShowTip(true);

    onChange?.(undefined);
    if (typeof v === 'boolean' || !v) return [];
    return (await GetLibraryQuery(v)).map((item: LibraryItem) => ({
      ...item,
      label: `【${item.libraryId}】${item.libraryName}`,
      value: `【${item.libraryId}】${item.libraryName}`,
    }));
  };
  ...
  return readonly ? (
    ...
    <>
      <AutoComplete
        ...
        // onSearch={run}
        placeholder="请输入资料编码或者名称检索"
        onSelect={(_, item: LibraryItem) => onChange?.(item.libraryId)}
      />
...
export default FormAutoCompleteLibrary;
