...
import { DeleteOutlined, EditOutlined, ExclamationCircleFilled, EyeInvisibleOutlined, EyeOutlined } from '@ant-design/icons';
import { ModalForm, ProFormRadio, ProFormSelect, ProFormTextArea } from '@ant-design/pro-components';
import { useSearchParams } from '@umijs/max';
import { Button, Checkbox, Col, Empty, Form, Image, message, Row, Space, Tabs, Tag } from 'antd';
    ...
    <div className={`${styles.detailBox} padding-tb-24`}>
      <header className="padding-lr-24">
        ...
        <div className={`${styles.headerContent} margin-left10`}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ color: '#262626', fontSize: 16, fontWeight: 600, marginRight: 4 }}>{doctorDetail?.doctorName}</div>
            {doctorDetail?.mobile && (
              <Fragment>
                {mobileFlag ? (
                  <div style={{ width: 90 }} className="font-14">
                    {doctorDetail?.mobile}
                  </div>
                ) : (
                  <div style={{ width: 90 }} className="font-14">
                    {`${doctorDetail?.mobile?.substring(0, 3)}****${doctorDetail?.mobile?.substring(7)}`}
                  </div>
                )}
          ...
          <div style={{ height: 20 }} className="font-14 color-text-45">
            {doctorDetail?.area}
          </div>
        </div>
        {/* 这期不做 */}
      ...
      <div className={styles.content}>
        ...
        <div className={`${styles.contentItem} padding-lr-24`}>
          ...
          <div style={{ display: 'flex' }}>
            <div className="color-title font-w6" style={{ minWidth: 70, height: 20 }}>
              状态：
            </div>
            <div>{doctorDetail?.status === 'ACTIVE' ? '有效' : doctorDetail?.status === 'INACTIVE' ? '无效' : ''}</div>
            <div>
              <Space>
                <ModalForm
                  width={400}
                  title="修改状态"
                  trigger={
                    <Button style={{ height: 20, display: 'flex', alignItems: 'center', marginTop: 2 }} type="link" icon={<EditOutlined />}>
                      编辑
                    </Button>
                  }
                  layout="horizontal"
                  ...
                  onFinish={async (values) => {
                    const res = await updateDoctorStatus({ status: values.status, doctorId: doctorDetail?.doctorId });
                    if (res) {
                      message.success('修改成功');
                      await queryGetDoctorDetail();
                    }
                    return true;
                  }}
                  labelCol={{ span: 4 }}
