...
// 全部客户
export async function searchDoctor(data: any) {
  return request(`/api/xiaoai/doctor/searchDoctor`, { method: 'POST', data });
}

// 导出筛选结果
export async function exportDoctor(data: any) {
  return request(`/api/xiaoai/doctor/exportDoctor`, { method: 'POST', data });
}
...
// 获取外部联系人
export async function contaceList(doctorId: string) {
  return request(`/api/xiaoai/doctor/contaceList?doctorId=${doctorId}`, { method: 'GET' });
}

// 审核状态
export async function verifyStatusList() {
  return request(`/api/xiaoai/doctor/verifyStatusList`, { method: 'GET' });
}
